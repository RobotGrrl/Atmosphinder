...
void AtmosphinderLogger::setLogData_t(int log_item, time_t val) {
  if(log_item == LOG_TIME) logdata.sample_time = val;
}

void AtmosphinderLogger::setLogData_u8(int log_item, uint8_t val) {
...
void AtmosphinderLogger::setLogData_u16(int log_item, uint16_t val) {
  ...
  if(log_item == LOG_WIND_SENSOR) logdata.wind_sensor = val;
...
void AtmosphinderLogger::setLogData_f(int log_item, float val) {
  ...
  if(log_item == LOG_GPS_SPEED) logdata.gps_speed = val;
  if(log_item == LOG_GPS_ANGLE) logdata.gps_angle = val;
  if(log_item == LOG_GPS_HDOP) logdata.gps_hdop = val;
  if(log_item == LOG_GPS_VDOP) logdata.gps_vdop = val;
  if(log_item == LOG_GPS_PDOP) logdata.gps_pdop = val;
  if(log_item == LOG_PRESSURE) logdata.pressure = val;
  if(log_item == LOG_HUMIDITY) logdata.humidity = val;
  if(log_item == LOG_TEMPERATURE) logdata.temperature = val;
}

void AtmosphinderLogger::setLogData_s(int log_item, String val) {
  if(log_item == LOG_AQ_DATA) logdata.aq_data = val;
}


void AtmosphinderLogger::randomLogData() {
  ...
  logdata.gps_speed = (float)random(0, 10);
  logdata.gps_angle = (float)random(0, 10);
  logdata.gps_hdop = (float)random(40, 42);
  logdata.gps_vdop = (float)random(40, 42);
  logdata.gps_pdop = (float)random(40, 42);
...
void AtmosphinderLogger::writeLogData() {
  ...
  logging_file.print(logdata.gps_speed, 1);
  logging_file.print(",");

  logging_file.print(logdata.gps_angle, 1);
  logging_file.print(",");

  logging_file.print(logdata.gps_hdop, 1);
  logging_file.print(",");

  logging_file.print(logdata.gps_vdop, 1);
  logging_file.print(",");

  logging_file.print(logdata.gps_pdop, 1);
  logging_file.print(",");

  logging_file.print(logdata.battery_sensor);
  logging_file.print(",");

  logging_file.print(logdata.wind_sensor);
  logging_file.print(",");
  ...
  logging_file.print(logdata.pressure, 2);
  logging_file.print(",");
...
void AtmosphinderLogger::logEvent(time_t sample_time, char action, char cmd1, uint8_t key1,
...
void AtmosphinderLogger::closeLogFile() {
  if(DEBUG_PRINTS) Serial.println("Closing log file");
  logging_file.close();
}

void AtmosphinderLogger::openLogFile() {
...
void AtmosphinderLogger::logFileTimer() {
...
void AtmosphinderLogger::createLogFile() {
